...
type DashboardNotice = GetDashboardHomeWorkspaceData["assistantMessages"][number];
const DASHBOARD_DUE_APPOINTMENT_IDS_KEY = "medassist_dashboard_due_appointment_ids";
const DOCTOR_NOTICE_IDS_KEY = "medassist_doctor_seen_notice_ids";
...
function readSeenDoctorNoticeIds() {
  if (typeof window === "undefined") {
    return [];
  }

  try {
    const raw = window.localStorage.getItem(DOCTOR_NOTICE_IDS_KEY) || "[]";
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) {
      return [];
    }
...
function persistSeenDoctorNoticeIds(ids: string[]) {
  if (typeof window === "undefined") {
    return;
  }

  try {
    window.localStorage.setItem(DOCTOR_NOTICE_IDS_KEY, JSON.stringify(ids));
  } catch {
    // Ignore storage failures in private mode or restricted environments.
  }
}
...
export default function DashboardPage() {
  ...
  const doctorNoticeThreadKey = buildDoctorNoticeThreadKey(getActiveDoctorUid());
  ...
  const [seenDoctorNoticeIds, setSeenDoctorNoticeIds] = useState<string[]>(() => readSeenDoctorNoticeIds());
      ...
      .then((response) => {
        ...
        setMessages(
          response.data.assistantMessages
            .filter((message) => message.threadKey === DEFAULT_DASHBOARD_THREAD)
            .map((message) => ({
              id: message.id,
              role: message.role,
  ...
  const doctorNoticeMessages = useMemo<DashboardNotice[]>(
    () =>
      (workspace?.assistantMessages ?? [])
        .filter((message) => message.threadKey === doctorNoticeThreadKey)
        .slice()
        .sort((left, right) => {
          const rightTime = new Date(right.createdAt || 0).getTime();
          const leftTime = new Date(left.createdAt || 0).getTime();
          return rightTime - leftTime || right.displayOrder - left.displayOrder;
        }),
    [doctorNoticeThreadKey, workspace?.assistantMessages]
  );
  const unreadDoctorNoticeCount = useMemo(
    () => doctorNoticeMessages.filter((notice) => !seenDoctorNoticeIds.includes(notice.id)).length,
    [doctorNoticeMessages, seenDoctorNoticeIds]
  );
  ...
  const notificationBadgeCount = Math.min(
    unreadDoctorNoticeCount + upcomingConsultations + highRiskCount,
    99
  );
  const notificationItems = useMemo(() => {
    ...
    if (highRiskCount > 0) {
      items.push({
        id: "notification-high-risk",
        title: `${highRiskCount} ca can luu y`,
        description: "Co phan tich vong mac nguy co cao can duoc uu tien xem lai.",
        meta: "Canh bao AI",
        tone: "red",
      });
    }
    ...
    if (upcomingConsultations > 0) {
      items.push({
        id: "notification-upcoming-consultations",
        title: `${upcomingConsultations} ca tu van sap toi`,
        description: nextUpcomingConsultation
          ? `${nextUpcomingConsultation.patient.displayName} luc ${formatShortTime(nextUpcomingConsultation.scheduledAt)}`
          : "Co lich tu van dang cho xu ly trong 24 gio toi.",
        meta: "Lich kham",
        tone: "blue",
      });
    }

    doctorNoticeMessages.slice(0, 3).forEach((notice) => {
      const isUnread = !seenDoctorNoticeIds.includes(notice.id);

      items.push({
        id: notice.id,
        title: isUnread ? "Thong bao dieu phoi moi" : "Thong bao dieu phoi",
        description: notice.content,
        meta: notice.timestampLabel,
        tone: isUnread ? "emerald" : "slate",
      });
    });

    return items;
  }, [
    doctorNoticeMessages,
    highRiskCount,
  ...
  useEffect(() => {
    if (typeof window === "undefined" || !doctorNoticeMessages.length) {
      return;
    }

    const unreadNotices = doctorNoticeMessages.filter((notice) => !seenDoctorNoticeIds.includes(notice.id));
    if (!unreadNotices.length) {
      return;
    }

    const nextSeenIds = Array.from(new Set([...seenDoctorNoticeIds, ...unreadNotices.map((notice) => notice.id)]));
    setSeenDoctorNoticeIds(nextSeenIds);
    persistSeenDoctorNoticeIds(nextSeenIds);

    const newestNotice = unreadNotices[0];
    if ("Notification" in window) {
      const showNotification = () => {
        try {
          new window.Notification("MedAssist - Điều phối mới", {
            body: newestNotice.content,
            tag: `doctor-notice-${newestNotice.id}`,
          });
        } catch {
          // Ignore browser notification errors and keep the in-app panel visible.
        }
      };
  ...
  return (
    <div className="mx-auto max-w-[1560px] space-y-5 2xl:max-w-[1640px]">
      <div className="relative overflow-hidden rounded-[30px] bg-slate-900 p-6 text-white sm:p-7 lg:p-8">
        ...
        <div className="relative z-10 flex flex-col items-start justify-between gap-5 lg:gap-6 xl:flex-row xl:items-center">
          <div className="flex items-center gap-4">
            {doctorProfile?.avatarUrl ? (
              <img
                src={doctorProfile.avatarUrl}
                alt={doctorProfile.fullName || "Doctor"}
                className="h-14 w-14 rounded-[18px] border-2 border-white/20 object-cover lg:h-16 lg:w-16"
              />
            ) : (
              <div className="flex h-14 w-14 items-center justify-center rounded-[18px] border-2 border-white/20 bg-white/10 text-base font-bold text-white lg:h-16 lg:w-16 lg:text-lg">
                BS
              </div>
            )}
            <div>
              <p className="text-[13px] font-semibold uppercase tracking-[0.22em] text-slate-300">Xin chào!</p>
              <h1 className="text-2xl font-bold leading-tight lg:text-[2rem]">
                {doctorProfile?.fullName || "Bác sĩ MedAssist"}
              </h1>
            </div>
          </div>

          <div className="flex w-full max-w-[920px] flex-1 items-center gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full rounded-full border border-white/20 bg-white/10 py-3 pl-12 pr-4 text-sm text-white placeholder-slate-400 backdrop-blur-md transition-all focus:bg-white/20 focus:outline-none focus:ring-2 focus:ring-blue-500 lg:py-3.5"
              />
            </div>
            <div ref={notificationPanelRef} className="relative shrink-0">
              <button
                type="button"
                onClick={() => setIsNotificationOpen((current) => !current)}
                className="relative flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-white/10 backdrop-blur-md transition-colors hover:bg-white/20 lg:h-12 lg:w-12"
              >
              ...
              {isNotificationOpen ? (
                <div className="absolute right-0 top-[calc(100%+14px)] z-30 w-[340px] rounded-[28px] border border-white/10 bg-[#10192d]/95 p-4 text-white shadow-[0_24px_60px_-24px_rgba(15,23,42,0.85)] backdrop-blur-xl">
                  <div className="mb-3 flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold">Thong bao dashboard</p>
                      <p className="text-xs text-slate-300">
                        {notificationBadgeCount
                          ? `${notificationBadgeCount} muc can ban xem`
                          : "Khong co cap nhat moi"}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setIsNotificationOpen(false)}
                      className="rounded-full border border-white/10 px-3 py-1 text-[11px] font-semibold text-slate-200 transition-colors hover:bg-white/10"
                    >
                      Dong
                    </button>
                  </div>
                  ...
                  <div className="space-y-2.5">
                    {notificationItems.length ? (
                      ...
                      <div className="rounded-2xl border border-dashed border-white/10 bg-white/5 px-3.5 py-4 text-sm text-slate-300">
                        Hien chua co thong bao nao can xu ly.
                      </div>
                    )}
                  </div>
                  ...
                  <div className="mt-3 flex justify-end">
                    <button
                      ...
                      className="rounded-full bg-white/10 px-4 py-2 text-xs font-semibold text-white transition-colors hover:bg-white/15"
                    >
                      Mo khu tu van
                    </button>
                  </div>
            ...
            <button
              ...
              className="flex shrink-0 items-center gap-2 rounded-full bg-blue-500 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-blue-500/30 transition-colors hover:bg-blue-600 lg:px-6"
            >
        ...
        <div className="relative z-10 mt-7">
          <div className="mb-3 flex items-center gap-2">
            <h2 className="text-lg font-bold lg:text-xl">Ưu tiên hôm nay</h2>
            <span className="flex h-2 w-2 rounded-full bg-blue-500"></span>
          </div>
          <div className="flex flex-wrap gap-3">
            <div className="flex items-center gap-3 rounded-[22px] border border-white/10 bg-white/10 px-4 py-2.5 text-sm backdrop-blur-md lg:px-5 lg:py-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-blue-500/20 text-blue-400">
                <Eye className="h-4 w-4" />
              </div>
              <span className="font-semibold">{highRiskCount} Phân tích võng mạc</span>
            </div>
            <button
              type="button"
              onClick={handleOpenUpcomingConsultations}
              className="flex items-center gap-3 rounded-[22px] border border-white/10 bg-white/10 px-4 py-2.5 text-sm backdrop-blur-md transition-colors hover:bg-white/20 lg:px-5 lg:py-3"
            >
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-400">
                <Calendar className="h-4 w-4" />
              </div>
              <span className="font-semibold">{upcomingConsultations} Ca tư vấn sắp tới</span>
            </button>
          </div>
        </div>

        <div className="absolute bottom-5 right-6 z-10 hidden text-right md:block">
      ...
      <div className="grid grid-cols-1 gap-5 xl:grid-cols-[minmax(0,1.48fr)_minmax(340px,1fr)_minmax(300px,0.82fr)]">
        <div className="space-y-5">
                ...
                liveQueuePatients.map((patient) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative min-w-[220px] cursor-pointer overflow-hidden rounded-[24px] bg-white p-4 shadow-sm transition-colors sm:min-w-[232px] ${
                    ...
                    >
                      ...
                      <h4 className="mb-1 truncate text-[1.05rem] font-bold text-slate-900">{patient.displayName}</h4>
          ...
          <div className="rounded-[30px] border border-slate-200 bg-white p-5 shadow-sm lg:p-6">
            ...
            <div className="mb-6 flex items-center gap-4 lg:gap-5">
              <button
                type="button"
                onClick={() => {
                ...
                className="flex h-16 w-16 items-center justify-center overflow-hidden rounded-full border-2 border-slate-200 bg-slate-100 text-slate-400 lg:h-20 lg:w-20"
              >
              ...
              <div className="min-w-0">
                <h2 className="mb-2 truncate text-xl font-bold text-slate-900 lg:text-[1.65rem]">
                  {activePatientName}
                </h2>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                <div className="flex items-center gap-4 rounded-2xl border border-slate-200 bg-white p-3.5 lg:p-4">
                ...
                <div className="flex items-center gap-4 rounded-2xl border border-emerald-200 bg-emerald-50 p-3.5 lg:p-4">
            ...
            <div className="mt-6 flex flex-wrap justify-end gap-3">
              ...
              <button
                type="button"
                className="rounded-xl bg-blue-600 px-5 py-3 font-medium text-white shadow-lg shadow-blue-600/20 transition-colors hover:bg-blue-700"
        ...
        <div className="flex h-auto flex-col xl:self-start">
          <div className="flex min-h-[540px] flex-col overflow-hidden rounded-[30px] border border-slate-200 bg-white shadow-sm xl:h-[640px] 2xl:h-[680px]">
            <div className="flex items-center justify-between border-b border-slate-100 bg-white p-4 lg:p-5">
              <div className="flex items-center gap-3">
            ...
            <div className="flex-1 space-y-4 overflow-y-auto bg-gradient-to-b from-slate-50 to-white p-4 lg:p-5">
                  ...
                  <div
                    className={
                      message.role === "assistant"
                        ? "rounded-2xl rounded-tl-sm border border-slate-200 bg-white px-4 py-3 text-sm leading-6 text-slate-700 shadow-sm"
                        : "rounded-2xl rounded-tr-sm bg-gradient-to-r from-blue-600 to-sky-500 px-4 py-3 text-sm leading-6 text-white shadow-md shadow-blue-600/15"
                    }
                  >
                    {message.content}
                  </div>
            ...
            <div className="border-t border-slate-100 bg-white p-3.5 lg:p-4">
        ...
        <div className="space-y-5 xl:self-start">
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-3">
              <div>
                <h3 className="font-bold text-slate-900">Thông báo điều phối</h3>
                <p className="mt-1 text-xs text-slate-500">
                  Nhận trực tiếp từ quản trị viên khi có ca cần ưu tiên xử lý.
                </p>
              </div>
              <span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700">
                {doctorNoticeMessages.length} thông báo{unreadDoctorNoticeCount ? ` • ${unreadDoctorNoticeCount} mới` : ""}
              </span>
            </div>

            {doctorNoticeMessages.length ? (
              <div className="space-y-3">
                {doctorNoticeMessages.slice(0, 3).map((notice) => (
                  <div key={notice.id} className="rounded-2xl border border-slate-200 bg-slate-50/80 p-4">
                    <div className="flex items-start justify-between gap-3">
                      <p className="text-sm font-semibold leading-6 text-slate-800">{notice.content}</p>
                      {!seenDoctorNoticeIds.includes(notice.id) ? (
                        <span className="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold uppercase tracking-wide text-emerald-700">
                          Mới
                        </span>
                      ) : null}
                    </div>
                    <p className="mt-2 text-xs text-slate-500">{notice.timestampLabel}</p>
                  </div>
                ))}
              </div>
            ) : (
              <div className="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-4 text-sm text-slate-500">
                Chưa có thông báo điều phối nào từ quản trị.
              </div>
            )}
          </div>
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div>
              ...
              <div className="relative space-y-5 before:absolute before:inset-0 before:-translate-x-px before:bg-gradient-to-b before:from-transparent before:via-slate-200 before:to-transparent before:h-full before:w-0.5 before:ml-2 md:before:mx-auto md:before:translate-x-0">
              ...
              <button
                type="button"
                onClick={() => router.push("/dashboard/schedule")}
                className="mt-6 w-full rounded-xl border border-slate-200 py-3 text-sm font-medium text-slate-600 transition-colors hover:bg-slate-50"
              >
