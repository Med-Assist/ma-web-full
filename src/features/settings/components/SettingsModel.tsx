...
export function SettingsModel() {
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <div className="flex justify-end space-x-4 pt-4"><AnimatedButton variant="outline" className="px-6 py-2.5" onClick={() => createSupportRequest(getMedAssistDataConnect(), { doctorUid: getActiveDoctorUid(), source: "settings", message: "Cần hỗ trợ kỹ thuật tại màn hình cài đặt.", createdAt: nowIsoString() }).then(() => alert("Đã gửi yêu cầu hỗ trợ.")).catch(() => alert("Không thể gửi yêu cầu hỗ trợ."))}>Gửi hỗ trợ</AnimatedButton><AnimatedButton variant="primary" className={`px-6 py-2.5 ${saving ? "opacity-75 cursor-not-allowed" : ""}`} onClick={saveAll}>{saving ? "Đang lưu..." : "Lưu tất cả cài đặt"}</AnimatedButton></div>
